...
import { ShoppingCart, Package, Menu, X, Wallet, MapPin, ChevronDown } from 'lucide-react'
...
export default function Navbar({ cartCount = 0, cashback = 0, onCartOpen }: NavbarProps) {
  ...
  const locationLabel = locationMounted
    ? (selectedState || 'Set Location')
    : 'Set Location'
    ...
    <>
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-brand-border shadow-nav">
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              className="md:hidden bg-white/95 backdrop-blur-xl border-t border-brand-border px-4 py-5 flex flex-col gap-1"
